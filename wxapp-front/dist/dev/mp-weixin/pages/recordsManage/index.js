...
const pages_recordsManage_columns = require("./columns.js");
...
if (!Math) {
  (_easycom_tm_text + _easycom_tm_alert + _easycom_tm_button + DataTable + AskModel + FormModel + _easycom_tm_app)();
}
...
const _sfc_main = /* @__PURE__ */ common_vendor.defineComponent({
  ...
  setup(__props) {
    ...
    const setTableData = (data) => {
      for (const c of data) {
        tableData.data.push(__spreadValues(__spreadProps(__spreadValues({}, c), {
          update: "修改",
          delete: "删除"
        }), pages_recordsManage_columns.getAttachmentOpts(pages_recordsManage_columns.statusToColor(c.status))));
      }
    };
    const initAllRecords = () => __async(this, null, function* () {
      tableData.data.length = 0;
      const data = yield pages_recordsManage_columns.getLocalData();
      setTableData(data);
    });
    const initRecord = () => {
      ...
      const dataKey = columns.find((item) => utils_columnsHandler.dateTag.includes(item.prop));
      if (dataKey) {
        record.value[dataKey.prop] = utils_dateFormat.formatReadableDate(/* @__PURE__ */ new Date());
      }
    };
    pages_recordsManage_columns.createColumnsAndTable(columns, tableData);
    pages_recordsManage_columns.initDB().then(() => pages_recordsManage_columns.initDBTable(columns)).then(() => pages_recordsManage_columns.getLocalData()).then((res) => {
      setTableData(res);
      flag.value = true;
    });
    initRecord();
    ...
    const hdSave = () => __async(this, null, function* () {
      const rec = pages_recordsManage_columns.getPureRecords(tableData, false);
      yield pages_recordsManage_columns.saveToLocal(rec);
    ...
    const uploadRecords = () => __async(this, null, function* () {
      ...
      yield pages_recordsManage_columns.afterUpload(rec);
      yield initAllRecords();
    ...
    const deleteExistRecord = () => __async(this, null, function* () {
      if (tableData.data[curDelete]["lid"] !== void 0) {
        yield pages_recordsManage_columns.deleteLocalData(tableData.data[curDelete]["lid"]);
      }
      tableData.data.splice(curDelete, 1);
    });
    const deleteAllRecord = () => __async(this, null, function* () {
      const rec = pages_recordsManage_columns.getPureRecords(tableData);
      yield pages_recordsManage_columns.deleteAllLocalData(rec);
      tableData.data.length = 0;
    });
    const updateExistRecord = () => {
      tableData.data.splice(curUpdate, 1, __spreadValues(__spreadProps(__spreadValues({}, record.value), {
        status: "未保存"
      }), pages_recordsManage_columns.getAttachmentOpts("red")));
      clearRecord();
    };
    const addRecord = () => {
      if (isAllBlank()) {
    ...
    return (_ctx, _cache) => {
      return common_vendor.e({
        ...
        o: common_vendor.o(deleteAllRecord),
        p: common_vendor.o(($event) => showClearModel.value = $event),
        q: common_vendor.p({
          title: "提示",
          content: "是否确认清空所有记录",
          ["check-color"]: "red",
        ...
        r: common_vendor.o(uploadRecords),
        s: common_vendor.o(($event) => showUploadModel.value = $event),
        t: common_vendor.p({
          title: "提示",
          content: "是否确认上传所有记录",
          ["check-color"]: "green",
        ...
        v: common_vendor.o(deleteExistRecord),
        w: common_vendor.o(($event) => showDeleteModel.value = $event),
        x: common_vendor.p({
          title: "提示",
          content: "是否确认删除该记录",
          ["check-color"]: "red",
        ...
        y: common_vendor.o(clearRecord),
        z: common_vendor.o(addRecord),
        A: common_vendor.o(toNewRecord),
        B: common_vendor.o(($event) => record.value = $event),
        C: common_vendor.o(($event) => showAddModel.value = $event),
        D: common_vendor.p({
          columns,
          title: "新建记录",
          ["add-record"]: true,
        ...
        E: common_vendor.o(clearRecord),
        F: common_vendor.o(updateExistRecord),
        G: common_vendor.o(($event) => showUpdateModel.value = $event),
        H: common_vendor.o(($event) => record.value = $event),
        I: common_vendor.p({
          title: "修改记录",
          columns,
...
const MiniProgramPage = /* @__PURE__ */ common_vendor._export_sfc(_sfc_main, [["__scopeId", "data-v-f3f09ad4"], ["__file", "D:/EquipmentManagerDev/EquipManager/wxapp-front/src/pages/recordsManage/index.vue"]]);
