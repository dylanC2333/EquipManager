...
export const getSheetsByFBGC = (name: string) => {
  const mid = sheetsMaps.find((item) => item.name === name)
  return mid?.sheets || []
}

export const getCurTableTitleByFBGC = () => {
  const mainStore = useMainStore()
...
export const getTrueFBGC = () => {
  const mainStore = useMainStore()
  const sheetName = mainStore.getCurType()
  const f = mainStore.projInfo.fbgc
  const res = sheetsMaps.find((item) => item.name === f)?.sheets.find((item) => item.title === sheetName)
  return res?.subclass || f
}
