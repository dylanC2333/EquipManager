...
import { attachStableData, dateTag } from '@/utils/columnsHandler'
...
const initRecord = () => {
  ...
  if (lastOne) {
    ...
    const needEchoProp = columns
      .filter((item) => item.echo)
      .map((item) => item.prop)
  ...
  if (dataKey) {
    record.value[dataKey.prop] = formatReadableDate(new Date())
  }
}
