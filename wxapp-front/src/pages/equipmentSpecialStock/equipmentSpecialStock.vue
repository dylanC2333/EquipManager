<template>
	<tm-text :font-size="48" _class="text-weight-b" label="特殊任务出入库"></tm-text>
	<tm-sheet>
<!-- 		<tm-text :font-size="30" _class="text-weight-b" label="请输入搜索关键字"></tm-text>
	...
	<tm-text :font-size="30" _class="text-weight-b" label="特殊任务出入库记录添加"></tm-text>
		<tm-button  color="green" :font-size="35" :margin="[20]" @click="add('sendtest')" size="normal">送检 </tm-button>
		<tm-button  color="blue" :font-size="35" :margin="[20]" @click="add('repair')" size="normal">维修 </tm-button>
		...
		>
			<tm-form ref="form" :label-width="80" @submit="confirm" v-model="sysEquipStock">
				...
				<tm-form-item required label="任务编号" field="taskCode" :rules="[{ required: true, message: '请正确填写任务编号', validator: validateTaskCode}]" >
					<tm-input disabled :inputPadding="[0, 0]"  v-model.lazy="taskCodeParts.year" :transprent="true" prefixLabel='RW-' placeholder="自动填写"> </tm-input>
					<tm-input disabled :inputPadding="[49, 0]" v-model.lazy="taskCodeParts.number" :transprent="true" prefixLabel='-' placeholder="自动填写"> </tm-input>
				</tm-form-item>
...
<script setup lang="ts">
	...
	// // 直接定义为一个整数类型
	// type SpecialTaskType = number;
	// // 使用类型
	// const specialTaskType = ref<SpecialTaskType>(0);
	...
	// 添加按钮
	const add = (specialTaskType:'sendtest'|'repair'|'practise') =>{
		...
		sysEquipStock.value.isTransfer = 2

		initialObject(taskCodeParts.value)
		taskCodeParts.value.number = '10001'
		switch(specialTaskType){
			case'sendtest':
				taskCodeParts.value.year = '送检'
				break;
			case'repair':
				taskCodeParts.value.year = '维修'
				break;
			case'practise':
				taskCodeParts.value.year = '练习'
				break;
		}
